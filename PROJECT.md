...
`ohlcv` contains the last N closed candles (N configurable, default 200) for indicator/context purposes. `position_context` tells the model only *whether* a position is currently open, so it can reason about exit conditions — it is a status flag, never a sizing or balance figure.

...
sequenceDiagram
    ...
    alt killswitch active
        ...
        alt signal.action = HOLD OR any rule fails
            ...
            RE->>FT: POST /forceenter { pair, side=long, stake_amount, entrytag=sl:<stop_loss_price> }
...
**Stop-loss is a true per-trade dynamic stop, wired through Freqtrade's `entry_tag`:** Freqtrade's `forceenter` API has no dedicated field for a per-trade custom stop-loss, but it does accept an opaque `entrytag` string that Freqtrade persists on the `Trade` row. The Risk Engine encodes the computed `stop_loss_price` as `sl:<price>` and passes it as `entrytag` (`risk_engine/app/main.py::_submit_entry_order` → `risk_engine/app/freqtrade_client.py::forceenter`). `ExternalSignalStrategy.custom_stoploss()` reads `trade.enter_tag` back, parses the price, and converts it to a relative stop via `stoploss_from_absolute()`. The strategy-wide static `stoploss` class attribute (set to `-max_stop_loss_pct`, the conservative upper bound) remains as the fallback whenever the tag is missing or fails to parse — rule 12's invariant ("every approved entry carries a stop") is satisfied by the tighter per-trade stop in the common case and the static stop in the fallback case, never by neither.

...
sequenceDiagram
    ...
    alt killswitch active and balance valid
        ...
        alt signal.action = HOLD OR any rule fails
            ...
            RE->>FT: POST /forceenter { pair, side=long, stakeamount, entry_tag=slpct:<stop_distance_pct> }
...
After structural validation, `validators/semantic.py` deterministically checks the model action against confirmation facts `ContextBuilder` already computed onto `MarketContext.entry_confirmations` / `exit_confirmations` (`context/builder.py`) — the supplied closed candles and indicators are read once, not re-derived here. Provider failures and structurally invalid responses never reach this step and remain `HOLD`. For a valid response:

- With no open position, a model-proposed `SELL` is normalized to `HOLD`.
- With no open position, `BUY` requires at least three numeric confirmations including both trend (price above EMA50 and EMA200; EMA50 above EMA200) and momentum (bullish MACD; RSI from 45 inclusive to 70 exclusive). Higher highs/lows and a rising close on above-average volume are additional price-action confirmations. A high-confidence model narrative that contradicts those values is normalized to `HOLD`.
- With an open position, a model-proposed `SELL` requires both (a) at least two confirmations spanning at least two categories — trend (price below EMA50 and EMA200; EMA50 below EMA200), momentum (bearish MACD; RSI below 45), price action (lower highs/lows; falling close on above-average volume) — and (b) known PnL outside the configured no-action cushion. Two confirmations from the same category do not satisfy the rubric.
- Semantic validation never promotes model `HOLD` to `BUY` or `SELL`. It may confirm a proposed action or normalize an unsafe/contradictory action to `HOLD`; it does not originate trading intent.
...
On approval, the Risk Engine calls Freqtrade's `forceexit` (not `forceenter`) against the open position's Freqtrade trade ID. Risk exits specify a market order and retry transport/API failures promptly within a bounded attempt count; entries are never retried because doing so could duplicate exposure.
...
**Stop-loss is a true per-trade dynamic stop, wired through Freqtrade's `entry_tag`:** Freqtrade's `forceenter` API has no dedicated per-trade stop field, but it accepts the opaque `entry_tag` property and persists it on the `Trade` row. The Risk Engine encodes `stop_distance_pct` as `slpct:<distance>` and passes it as `entry_tag` (`risk_engine/app/main.py::_submit_entry_order` → `risk_engine/app/freqtrade_client.py::forceenter`). `ExternalSignalStrategy.custom_stoploss()` reads `trade.enter_tag`, derives the absolute stop from the authoritative filled `trade.open_rate`, and converts it with `stoploss_from_absolute()`. The synchronous force-entry response must echo the expected tag; a missing/mismatched tag marks submission failed and triggers an immediate market exit attempt. Legacy `sl:<absolute price>` tags remain readable for already-open trades. The strategy-wide static `stoploss` remains a final fallback for malformed legacy state, never the expected path for a new approved entry.
...
| Freqtrade API unreachable at approval time | Entry: `RiskDecision(approved=true)` is written, `Order` is written as `FAILED`, and Telegram alerts immediately; entries are never retried. Exit: the market `forceexit` is retried promptly for a bounded number of attempts, then persisted as `FAILED` and alerted if still unavailable |
